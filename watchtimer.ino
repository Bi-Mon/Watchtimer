 ...
 *
 * Taster-Verhalten:
 *   Kurzer Druck waehrend eines Alarms: Alarm stoppen, Countdown
 *   startet automatisch neu.
 *   Langer Druck (> 1,2s), unabhaengig vom Zustand: schaltet den
 *   Timer komplett aus bzw. wieder ein. Im ausgeschalteten
 *   Zustand laeuft kein Countdown, es wird kein Alarm ausgeloest,
 *   Display und Weboberflaeche zeigen "--:--". Der Encoder ist in
 *   diesem Zustand deaktiviert (wie auch waehrend eines Alarms).
 *
 * Verdrahtung:
 *   Buzzer  +  -> GPIO 25
 *   Buzzer  -  -> GND
...
// ----- Zustands-/Ereignis-Typen (muessen VOR jeder Funktion stehen, die sie
// als Rueckgabetyp nutzt - die Arduino-IDE generiert Funktions-Prototypen
// automatisch ganz oben in der Datei, noch vor spaeteren Typdefinitionen,
// was sonst zu "does not name a type"-Fehlern fuehrt) -----------------------
enum State { OFF, COUNTING, ALARMING };
...
const uint32_t LONG_PRESS_MS = 1200;   // Schwelle fuer "langer Druck"

// ----- Zustände ------------------------------------------------------------
State state = COUNTING;   // Timer ist nach dem Booten standardmaessig eingeschaltet
...
// Taster-Entprellung + Lang-/Kurzdruck-Erkennung
...
// Liefert BTN_SHORT_PRESS bzw. BTN_LONG_PRESS genau einmal pro Tastendruck,
// unabhängig vom aktuellen Zustand - wird zentral aus loop() aufgerufen.
// Lang-Druck loest bereits beim Erreichen der Schwelle aus (waehrend der
// Taster noch gehalten wird), damit die Reaktion nicht erst beim Loslassen
// kommt; ein kurzer Druck wird erst beim Loslassen gemeldet.
ButtonEvent pollButton() {
  ...
  if ((millis() - lastEdgeAt) > DEBOUNCE_MS && raw != stableState) {
    stableState = raw;
    if (stableState == LOW) {
      // Taster gegen GND = gerade gedrueckt
      buttonIsDown = true;
      buttonDownAt = millis();
      longPressFired = false;
    } else {
      // gerade losgelassen
      if (buttonIsDown && !longPressFired) {
  ...
  if (buttonIsDown && !longPressFired && (millis() - buttonDownAt) >= LONG_PRESS_MS) {
    longPressFired = true;
...
// Schaltet den Timer ein (Countdown startet neu). "source" nur fuer die
// Log-Ausgabe, z.B. "Taster" oder "Web".
void turnTimerOn(const char* source) {
  Serial.println("[Timer] Eingeschaltet (" + String(source) + ").");
  startCountdown();
  publishState();
  publishSignalK();
}

// Schaltet den Timer komplett aus (Buzzer/LED aus, kein Countdown/Alarm
// mehr, bis er wieder eingeschaltet wird). "source" nur fuer die Log-
// Ausgabe.
void turnTimerOff(const char* source) {
  Serial.println("[Timer] Ausgeschaltet (" + String(source) + ").");
  digitalWrite(BUZZER_PIN, LOW);
  digitalWrite(LED_PIN, LOW);
  buzzerOn = false;
  state = OFF;
...
// Schaltet den Timer um (aus dem physischen langen Tastendruck genutzt).
void toggleTimerOnOff(const char* source) {
  if (state == OFF) {
    turnTimerOn(source);
  } else {
    turnTimerOff(source);
...
// Timer ist manuell ausgeschaltet: keine Aktion noetig, LED bleibt aus,
// Encoder wird (wie waehrend eines Alarms) ignoriert.
void handleOff() {
}
...
void drawOffScreen() {
  ...
  u8g2.setFont(u8g2_font_6x10_tf);
  const char* label = "AUSGESCHALTET";
  int labelW = u8g2.getStrWidth(label);
  u8g2.drawStr((128 - labelW) / 2, 14, label);

  // _tf (volle Zeichensatzbreite) statt _tn, da der reine Ziffern-Subset
  // moeglicherweise kein "-" enthaelt
  u8g2.setFont(u8g2_font_logisoso32_tf);
  const char* txt = "--:--";
  int w = u8g2.getStrWidth(txt);
  u8g2.drawStr((128 - w) / 2, 46, txt);

  u8g2.setFont(u8g2_font_6x10_tf);
  const char* sub = "langer Druck = an";
  int subW = u8g2.getStrWidth(sub);
  u8g2.drawStr((128 - subW) / 2, 62, sub);
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .power-btn{ width:100%; max-width:340px; margin:14px 0 4px; }
  .power-btn.is-off{ border-color:var(--amber); color:var(--amber); }
</style>
</head>
<body>
  <h1>Wach-Timer</h1>
  <div id="state">-</div>
  <div id="clock">--:--</div>
  <button id="powerBtn" class="power-btn">Timer ausschalten</button>
...
<script>
  ...
  async function poll(){
    try{
      ...
      const isAlarm = d.state === 'ALARM';
      stateEl.textContent = isAlarm ? 'ALARM' : (isOff ? 'AUS' : 'LÄUFT');
      clockEl.textContent = isAlarm ? 'ALARM' : (isOff ? '--:--' : fmt(d.remainingSeconds));
      clockEl.style.color = isAlarm ? '#ff4433' : (isOff ? '#6b7580' : '#e8a33d');
      if (!editingMinutes) minutesEl.value = d.intervalMinutes;
      currentIntervalMinutes = d.intervalMinutes;
      stopBtn.disabled = !isAlarm;

      powerBtn.textContent = isOff ? 'Timer einschalten' : 'Timer ausschalten';
      powerBtn.classList.toggle('is-off', isOff);
      powerBtn.dataset.off = isOff ? '1' : '0';
  ...
  powerBtn.addEventListener('click', async () => {
    const turningOn = powerBtn.dataset.off === '1';
    await fetch('/power', {
      method:'POST',
      headers:{'Content-Type':'application/x-www-form-urlencoded'},
...
void handleStatus() {
  String stateStr = (state == ALARMING) ? "ALARM" : (state == OFF ? "OFF" : "COUNTING");
  String json = "{";
  json += "\"state\":\"" + stateStr + "\",";
...
// Schaltet den Timer über die Weboberfläche aus/ein (Parameter "on"=1/0).
// Wirkt unabhängig vom aktuellen Zustand, genau wie der lange Tastendruck -
// also auch waehrend eines laufenden Alarms als Not-Aus nutzbar.
void handlePower() {
  if (!server.hasArg("on")) {
    server.send(400, "text/plain", "on fehlt");
    return;
  }
  bool wantOn = server.arg("on") == "1";
  if (wantOn && state == OFF) {
    turnTimerOn("Web");
  } else if (!wantOn && state != OFF) {
    turnTimerOff("Web");
  }
  server.send(200, "text/plain", "OK");
}
...
void loop() {
  ...
  // Taster zentral auswerten, unabhaengig vom Zustand (fuer den langen
  // Druck zum Aus-/Einschalten), kurzer Druck wirkt nur waehrend ALARMING
  ButtonEvent btnEvent = pollButton();
  if (btnEvent == BTN_LONG_PRESS) {
    toggleTimerOnOff("Taster");
  } else if (btnEvent == BTN_SHORT_PRESS && state == ALARMING) {
